...
def _cleanup_tmpfs_exit():
    ...
    if _CURRENT_SESSION_DIR and _CURRENT_SESSION_DIR.exists():
        shutil.rmtree(_CURRENT_SESSION_DIR, ignore_errors=True)
...
class AutomatedHvOpEngine:
    def __init__(self, target_directory: str, output_directory: str = ""):
        ...
        # Caches (FIXED: prevents redundant decodes)
    ...
    def _get_mismatch_path(self, src: Path, suffix: str) -> Path:
        ...
        dest_dir.mkdir(parents=True, exist_ok=True)
        # Always .mkv for mismatch outputs
    ...
    def _is_playable(self, path: Path) -> bool:
        """Check if file is playable (FIXED: no redundant wrapper)."""
        key = str(path)
    ...
    def _pick_opus_tier(self, br_kbps: float, channels: int = 2) -> Dict[str, str]:
        ...
        # Boost one tier for surround (>=6 channels)
        if channels >= 6 and base_idx < len(OPUS_LADDER) - 1:
            return OPUS_LADDER[base_idx + 1]
        return OPUS_LADDER[base_idx]

    def _is_source_already_opus(self, audio_streams: List[Dict[str, Any]]) -> bool:
        """Check if ALL audio streams are already Opus (avoid lossy→lossy re-encode)."""
        if not audio_streams:
            return False
        return all(
            (s.get('codec_name') or '').lower() == 'opus'
    ...
    # -----------------------------------------------------------------------
    # HEVC video-only encode (tmpfs) — FIXED: readline instead of read(1)
    # -----------------------------------------------------------------------
    def _encode_hevc_video(self, src: Path, video_streams: List[Dict[str, Any]],
        ...
        try:
            ...
            # FIXED: use readline instead of read(1)
            for line in iter(process.stderr.readline, ''):
                line = line.strip()
                if "time=" in line:
                    parts = line.split()
                    time_part = [p for p in parts if p.startswith("time=")]
                    speed_part = [p for p in parts if p.startswith("speed=")]
    ...
    def _mux_candidate(self, src: Path, tmp_video: Path,
        ...
        for a_idx, track in enumerate(audio_streams):
            ...
            if is_opus:
                ...
                # FIXED: all options have a: prefix
                cmd.extend([
                    "-map", f"1:a:{a_idx}",
                    f"-c:a:{a_idx}", "libopus",
                    f"-b:a:{a_idx}", tier["br"],
    ...
    # -----------------------------------------------------------------------
    # Generate candidates — skip Opus if source already Opus
    # -----------------------------------------------------------------------
    def _mux_audio_candidates(self, src: Path, tmp_video: Path,
        ...
        # Skip Opus re-encode if source is already Opus (lossy→lossy waste)
        if self._is_source_already_opus(audio_streams):
            console.print("  [dim]Source audio already Opus — skipping Opus candidate[/]")
        ...
        try:
            ...
            if duration > 60:
                ...
                if sample_duration <= 0:
                    sample_duration = 30
                # FIXED: input-level -ss (fast seek) — much faster, negligible
                # accuracy loss for black-scene detection
                cmd = [
                    _FFMPEG, "-v", "error",
            ...
            # FIXED: log the failure instead of silently returning 0.0
    ...
    def _check_gate345(self, src: Path, src_meta: Dict[str, Any],
        ...
        # GATE-4: Integrity (FIXED: no redundant cache wrapper)
        ...
        if src_v:
            ...
            if out_black >= 0.05 and out_black > src_black + 0.02:
                # FIXED: return False + "GATE-5" (consistent with hard-fail pattern)
                return False, "GATE-5", (
    ...
    def _verify_early_gates(self, src: Path, src_meta: Dict[str, Any],
        ...
        # GATE-7 (lightweight — threshold check only, no diagnostics)
        ...
        # GATE-3 → GATE-4 → GATE-5 (always run; caller decides routing)
    ...
    def _gate7_diagnostics(self, src: Path, src_meta: Dict[str, Any],
        ...
        """Run full diagnostics on mismatch save. Returns accumulated errors."""
        max_delta = mismatch_info["max_delta"]
    ...
    def _save_to_vault(self, src: Path, staged: Path, encoder_tag: str,
        ...
        """Move staged output to vault tree and handle source disposition."""
    ...
    def _save_to_mismatch(self, src: Path, staged: Path, encoder_tag: str,
        ...
        """Save mismatch candidate to mismatched_vault. Source ALWAYS kept."""
        ...
        # Run full diagnostics (only on save — avoids 12× decode storm)
    ...
    def run(self) -> None:
        ...
        for idx, f in enumerate(source_files):
            ...
            # Clean up tmp_video (no longer needed)
            if tmp_video.exists():
                tmp_video.unlink()
            ...
            # --- Case C: Both clean AND mismatch (dual-candidate GATE-6) ---
                ...
                # Clean wins if: passes GATE-6 AND (mismatch fails OR clean is smaller)
                ...
                # Mismatch wins if: passes GATE-6 AND (clean fails OR mismatch is smaller)
                elif mm_pass:
