#!/usr/bin/env python3
"""
HV_OP - V2.9.2: tmpfs Pipeline + Exhaustive Audio Mixing + Tree-Rebuilding Vault
------------------------------------------------------------------------------------
...
Changes from V2.9.0:
             ...
             - Fixed cross-filesystem black_hole crash (utime/metadata fallback)
"""
...
def _black_hole(path: Path) -> None:
    ...
    try:
        # Try native move first (fast, uses inode rename if on same filesystem)
        shutil.move(str(path), str(dest))
    except OSError:
        # Fallback: If move fails (cross-filesystem or restricted mount 
        # rejecting utime/metadata changes), copy raw data and delete source.
...
class AutomatedHvOpEngine:
    ...
    def _mux_candidate(self, src: Path, tmp_video: Path, audio_streams: List[Dict[str, Any]],
        ...
        for s_idx in range(subtitle_count):
            cmd.extend(["-map", f"1:s:{s_idx}", "-c:s:s:{s_idx}", "copy"])

        for t_idx in range(attachment_count):
            cmd.extend(["-map", f"1:t:{t_idx}", "-c:t:t:{t_idx}", "copy"])
    ...
    def _mux_all_audio_combinations(self, src: Path, tmp_video: Path,
        ...
        if 2 ** n > MAX_COMBINATIONS:
            ...
            smart_combos.append(tuple([0] * n)) # All copy

            for i in range(n): # Single track opus
                combo = [0] * n
                combo[i] = 1
    ...
    def run(self) -> None:
        ...
        console.print(Panel(
            f"[bold green]▶ HV_OP V2.9.2 — TMPFS PIPELINE + EXHAUSTIVE AUDIO MIX + TREE VAULT[/]\n"
        ...
        table = Table(
            title="HV_OP V2.9.2 — tmpfs Pipeline + Exhaustive Audio + Tree Vault Matrix",
