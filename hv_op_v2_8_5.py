#!/usr/bin/env python3
"""
HV_OP - V2.8.5: GPU-First (VAAPI) HEVC Encode + Dual Audio Race + 6-Gate Verification
------------------------------------------------------------------------------------
Philosophy : Absolute automation. Detects the Intel iGPU once at startup via a
             real VAAPI test-encode (not just a device-existence check). Every
             file then tries hevc_vaapi first; a failed GPU encode discards its
             partial tmp output and falls back to CPU libx265 for that file.
             Winner of opus-vs-copy audio race verified through 6 sequential
             gates before source fate is decided. Black hole dir permanently
             destroys unplayable outputs.

Naming     : The vault winner keeps its clean original filename
             ({stem}_vault.mkv). A uuid suffix is only appended if that name
             already exists in universal_vault/ (collision).
Discards   : The video-only intermediate ("a"), the losing audio candidate
             ("b" or "c"), and any staged output that fails a hard gate are
             all routed through the black hole dir rather than deleted
             in-place, so the reaper watchdog handles final cleanup.
"""
...
def _detect_gpu_backend() -> str | None:
    """
    Scan /dev/dri/renderD* for a usable Intel VAAPI render node and confirm
    ffmpeg can actually init_hw_device + hevc_vaapi encode through it —
    not just that the device file exists. Runs once at engine startup.

    Returns the render device path (e.g. '/dev/dri/renderD128') on success,
    or None if no working GPU backend is found (engine then runs CPU-only).
    """
...
def _clean_dest(dest_dir: Path, stem: str, suffix_tag: str, ext: str) -> Path:
    """
    Build a destination path using the clean original name (no uuid).
    Only falls back to a uuid-suffixed name if the clean name already
    exists in dest_dir (collision).
    """
    clean = dest_dir / f"{stem}_{suffix_tag}{ext}"
    if not clean.exists():
        return clean
    return dest_dir / f"{stem}_{uuid.uuid4().hex[:8]}_{suffix_tag}{ext}"
...
class AutomatedHvOpEngine:
    def __init__(self, target_directory: str, output_directory: str = ""):
        ...
        # One-time GPU backend probe for the whole session (not per-file).
    ...
    def _encode_hevc_video(self, src: Path, video_streams: list, use_fallback: bool) -> Path | None:
        ...
        for v_idx in range(len(video_streams)):
            if use_fallback:
                ...
                cmd_args.extend([
                    "-map", f"0:v:{v_idx}",
                    f"-c:v:{v_idx}", "hevc_vaapi", "-global_quality", "25",
                    f"-vf:v:{v_idx}", "format=nv12,hwupload,scale_vaapi=w=trunc(iw/2)*2:h=trunc(ih/2)*2"
                ])
        ...
        if use_fallback:
            ...
            base = [_FFMPEG, "-y",
                     "-init_hw_device", f"vaapi=hw:{self.gpu_device}",
                     "-filter_hw_device", "hw"]
    ...
    def run(self):
        ...
        console.print(Panel(
            f"[bold green]▶ HV_OP V2.8.5 — GPU-FIRST DUAL AUDIO RACE + 6-GATE SEQUENTIAL VERIFICATION[/]\n"
        ...
        for idx, f in enumerate(source_files):
            ...
            # --- Stage 1: Encode HEVC video-only (GPU VAAPI → CPU fallback) ---
            t0 = time.time()
            if self.gpu_device:
                console.print("  [cyan]Stage 1: Encoding HEVC video (VAAPI GPU)...[/]")
                tmp_video = self._encode_hevc_video(f, video_streams, use_fallback=False)
                use_fallback = False

                if tmp_video is None:
                    # Partial/failed GPU output is already discarded inside
                    # _encode_hevc_video (tmp_video.unlink() on exception).
                    console.print("  [yellow]GPU encode failed. Discarded partial output, falling back to CPU (libx265)...[/]")
                    tmp_video = self._encode_hevc_video(f, video_streams, use_fallback=True)
                    use_fallback = True
            else:
                console.print("  [cyan]Stage 1: Encoding HEVC video (CPU libx265 — no GPU backend this session)...[/]")
                tmp_video = self._encode_hevc_video(f, video_streams, use_fallback=True)
                use_fallback = True
            ...
            # Done with tmp video ("a") — retired to black hole, not deleted directly
            _black_hole(tmp_video)

            winner, audio_decision = self._pick_audio_winner(opus_path, copy_path)

            # Loser candidate ("b" or "c") — retired to black hole, not deleted directly
            ...
            if not passed:
                # Any hard gate failure (unplayable, zero/corrupt, size not reduced,
                # duration/dimension mismatch, etc.) — staged winner goes to black
                # hole. Source is always left untouched here.
            ...
            # All 6 gates passed — promote from prcs_file/ to universal_vault/
            out_sz = staged_output.stat().st_size
            potato = self._potato_tier(src_size, out_sz)
            final_output = _clean_dest(self.vault_dir, f.stem, "vault", ".mkv")
        ...
        table = Table(
            title="HV_OP V2.8.5 — GPU-First Encode + Dual Audio Race + 6-Gate Verification Matrix",
